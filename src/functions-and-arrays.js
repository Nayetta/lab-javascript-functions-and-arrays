...
function findLongestWord(theWord) {
  let longWord = "";
  let maxword = 0;

  for (let numWord = 0; numWord < theWord.length; numWord++) {
    if (theWord[numWord].length > maxword) {
      maxword = theWord[numWord].length;
      longWord = theWord[numWord];
    }
  }
  return longWord;
}
...
console.log(`El resultat de la suma és ` + sumNumbers(numbers));
...
console.log(`El resultat de la suma és ` + sum(mixedArr));
